...
# Constants
AUTHOR_URL = "https://app.stocks.news/blogs/author/stocks-news"
CHECK_INTERVAL = 0.1
PROCESSED_JSON_FILE = "data/stocknews_author_html_processed_urls.json"
...
async def send_match_to_telegram(url, stock_symbol, post_title):
    ...
    message = f"<b>New Stock Match Found - Author HTML</b>\n\n"
...
async def send_posts_to_telegram(entries, timestamp):
    ...
    joined_url_titles = "\n  ".join(url_titles)
    message = f"<b>New Posts Found - Author HTML</b>\n\n"
    message += f"<b>Time:</b> {timestamp}\n"
    message += f"<b>URLS:</b>\n  {joined_url_titles}"
    await send_telegram_message(message, TELEGRAM_BOT_TOKEN, TELEGRAM_GRP)
