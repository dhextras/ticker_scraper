...
# Constants
BASE_URL = "https://jehoshaphatresearch.com/wp-json/wp/v2/media"
START_ID = 651
    ...
    try:
        ...
        async with session.get(url, headers=headers, cookies=cookies) as response:
            if response.status == 200:
                ...
                if not cookies:
                    raise Exception("CF_CLEARANCE Failed: Media")
                return None, cookies
            elif response.status == 404:
                log_message(f"Media ID {post_id} not found", "INFO")
                return None, None
...
async def send_post_to_telegram(post_data: Dict) -> None:
    ...
    media_id = post_data.get("id", "Unknown ID")
    media_date = post_data.get("date", "Unknown Date")
    source_url = post_data.get("source_url", "No media URL")
    post_url = post_data.get("link", "No Post Link")

    message = f"<b>New Jehoshaphat Research Media - Media ID</b>\n\n"
    message += f"<b>Time Found:</b> {timestamp}\n"
    message += f"<b>Media ID:</b> {media_id}\n"
    message += f"<b>Media Date:</b> {media_date}\n"
    message += f"<b>Title:</b> {title}\n"

    if ticker != "Unknown":
        message += f"<b>Possible Ticker:</b> {ticker}\n"

    message += f"<b>Media URL:</b> {source_url}\n"
    message += f"<b>Post URL:</b> {post_url}\n"

    await send_telegram_message(message, TELEGRAM_BOT_TOKEN, TELEGRAM_GRP)
    log_message(f"Media ID {media_id} sent to Telegram", "INFO")
