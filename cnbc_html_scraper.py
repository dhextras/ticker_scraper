    ...
    try:
        ...
        for column in columns:
            ...
            for module in modules:
                        ...
                        articles.append(
                            {
                                ...
                                "url": f"https://www.cnbc.com{asset.get('url')}",
                                "type": "trade_alert",
                                "datePublished": asset.get("datePublished"),
                            }
                        )

                # Check for four up stack
                elif module.get("name") == "fourUpStack":
                    stack_assets = module.get("data", {}).get("assets", [])
                ...
                # Check for dynamic headline list
                elif module.get("name") == "dynamicHeadlineList":
                    list_assets = module.get("data", {}).get("assets", [])
    ...
    try:
        ...
        for alert in current_alerts:
            ...
            if alert_id not in alert_set:
                ...
                message = (
                    f"<b>New {alert['type'].replace('_', ' ').title()}!</b>\n"
                    f"<b>Title:</b> {alert['title']}\n"
                    f"<b>Link:</b> {alert['url']}\n"
                    f"<b>Published:</b> {alert['datePublished']}"
                )
