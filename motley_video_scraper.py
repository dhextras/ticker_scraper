...
PROCESSED_URLS_FILE = "data/motley_video_processed.json"
...
def save_processed_urls(processed_data):
...
async def fetch_latest_videos(session_data):
        ...
        try:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    log_message(f"Error fetching videos: {response.status}", "ERROR")
                    return []
        except Exception as e:
            log_message(f"Error in fetch_latest_videos: {e}", "ERROR")
            return []
...
async def process_video(video, known_instruments):
    try:
        current_time = get_current_time()
        publish_time = datetime.fromisoformat(
            video["publishAt"].replace("Z", "+00:00")
        ).astimezone(pytz.timezone("America/Chicago"))
        product_name = PRODUCT_NAMES.get(video["productId"], "Unknown")

        new_instruments = []
        if video["url"] in known_instruments:
            existing_ids = set(known_instruments[video["url"]])
            for instrument in video["instruments"]:
                if instrument["instrumentId"] not in existing_ids:
                    new_instruments.append(instrument)
        else:
            new_instruments = video["instruments"]

        if new_instruments or video["url"] not in known_instruments:
            instruments_text = (
                "\n".join(
                    [
                        f"    • {inst['name']} ({inst['symbol']}) - ID: {inst['instrumentId']}"
            ...
            message = (
                f"<b>New {product_name} Video!</b>\n"
                f"<b>Headline:</b> {video['headline']}\n"
                f"<b>URL:</b> {video['url']}\n"
                f"<b>Published:</b> {publish_time.strftime('%Y-%m-%d %H:%M:%S %Z')}\n"
        ...
        log_message(f"Error processing video: {e}", "ERROR")
    return False


async def check_for_new_videos(prev_videos, session_data):
    try:
        start_time = time.time()
        current_videos = await fetch_latest_videos(session_data)

        duration = time.time() - start_time
        log_message(
            f"fetch_videos took {duration:.2f} seconds. Total videos: {len(current_videos)}",
            "INFO",
        )
        ...
        known_urls = processed_data["urls"]
        new_or_updated = False

        for video in current_videos:
            url = video["url"]
            current_instruments = [
                inst["instrumentId"] for inst in video["instruments"]
            ]

            if url not in known_urls:
                await process_video(video, known_urls)
                known_urls[url] = current_instruments
                new_or_updated = True
            else:
                if set(current_instruments) != set(known_urls[url]):
                    await process_video(video, known_urls)
                    known_urls[url] = list(set(known_urls[url] + current_instruments))
                    new_or_updated = True

        if new_or_updated:
            processed_data["urls"] = known_urls
            save_processed_urls(processed_data)

        return current_videos

    except Exception as e:
        log_message(f"Error in check_for_new_videos: {e}", "ERROR")
        return prev_videos


async def run_video_monitor():
    prev_videos = []
