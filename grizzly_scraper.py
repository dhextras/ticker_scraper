...
async def setup_chrome_driver(headless=True, cookies=None):
    options = Options()
...
async def get_new_session():
    ...
    driver = await setup_chrome_driver(headless=False)
...
async def fetch_json(session, cookies):
    try:
        driver = await setup_chrome_driver(headless=False, cookies=cookies)
        driver.get(JSON_URL)
